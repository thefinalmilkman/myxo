#!/usr/bin/env node
'use strict';
// nx.js — the Nx entry point. `node nx.js --help` for the full CLI.
//   run a file: node nx.js script.nx [--trace] [--strict]   ·   subcommands: run · fmt · test · lsp · repl
//   --version / --help   ·   no args = REPL
//   embed: require('./nx').run(src, opts) / runTests(src, opts) — Nx inside any Node host (the Nexus)
...
let STD_CACHE = null;
function stdSource() {
  if (STD_CACHE == null) STD_CACHE = fs.readFileSync(path.join(__dirname, 'std.nx'), 'utf8');
  return STD_CACHE;
}
...
// Self-hosted: a .nx file tests itself in Nx, with no node:test involved.
...
function repl() {
  ...
  process.stdout.write(`Nx ${require('./package.json').version} — the language of the Nexus. Its runtime is a living law.\n`);
  ...
  rl.on('line', (line) => {
    ...
    if (src) {
      try {
        ...
        process.stdout.write((e instanceof NxError ? e.format() : `error: ${e.message}`) + '\n');
      }
    }
    rl.prompt();
  });
...
function fmtFiles(argv) {
  ...
  if (!files.length) usageError('usage: nx fmt <file...> [--check] [--write] [--drop-comments]');
  let dirty = false;
  for (const file of files) {
    let st;
    try { st = fs.statSync(file); } catch { throw new NxError(`no such file: ${file}`); }
    if (!st.isFile()) throw new NxError(`not a file: ${file}`);
    const src = fs.readFileSync(file, 'utf8');
...
// `nx test <file-or-dir...>` — run Nx test files, print a report, exit non-zero on any failure.
function testFiles(argv) {
  const targets = argv.filter(a => !a.startsWith('--'));
  if (!targets.length) usageError('usage: nx test <file-or-dir...>');
  const files = [];
  const collect = (p) => {
    if (!fs.existsSync(p)) throw new NxError(`no such path: ${p}`);
    const st = fs.statSync(p);
    if (st.isDirectory()) { for (const f of fs.readdirSync(p).sort()) collect(path.join(p, f)); }   // recursive
    else if (st.isFile() && p.endsWith('.nx')) files.push(p);                                        // a dir named *.nx is traversed, not read
  };
  for (const t of targets) collect(t);
  if (!files.length) throw new NxError('no .nx test files found');
  let pass = 0, fail = 0;
  for (const file of files) {
...
const USAGE = `Nx — the language of the Nexus.

Usage:
  nx <file.nx> [--trace] [--strict]   run a program  (--trace prints the mesh; --strict enforces types)
  nx run <file.nx> [--trace|--strict] run a program (explicit form)
  nx test <file|dir...>               run Nx test files (strict by default)
  nx fmt <file...> [--check|--write|--drop-comments]   format source
  nx lsp                              start the stdio language server
  nx plan <file.nx>                   preview a script's capability reach (the fence approval surface)
  nx repl                             start the REPL  (also: nx with no args)
  nx --version | -v                   print the version
  nx --help    | -h                   print this help`;

function usageError(msg) { process.stderr.write(`nx: ${msg}\n`); process.exit(2); }   // 2 = bad usage

function planFile(argv) {
  const bad = argv.find(a => a.startsWith('--'));
  if (bad) usageError(`unknown flag '${bad}' — try \`nx plan <file.nx>\``);
  const files = argv.filter(a => !a.startsWith('--'));
  if (!files.length) usageError('nx plan needs a file — try `nx plan <file.nx>`');
  ...
  catch { process.stderr.write(`nx: no such file: ${files[0]}\n`); process.exit(2); }
  if (!st.isFile()) { process.stderr.write(`nx: not a file: ${files[0]}\n`); process.exit(2); }
  const { planScript, formatPlan } = require('./nx-plan');
  let plan;
  try { plan = planScript(fs.readFileSync(files[0], 'utf8')); }
  catch (e) { process.stderr.write((e instanceof NxError ? e.format() : `nx: ${e.message}`) + '\n'); process.exit(2); }  // parse/analysis failure != policy
...
function main() {
  ...
  try {
    ...
    if (cmd === 'lsp') { require('./nx-lsp').serve(); return; }
    ...
    if (badFlag) usageError(`unknown flag '${badFlag}' — try \`nx --help\``);   // never silently drop a flag (e.g. a typo'd --strict)
    const files = args.filter(a => !a.startsWith('--'));
    if (!files.length) usageError('no input file — try `nx --help`');
    let st;
    try { st = fs.statSync(files[0]); }
    catch { process.stderr.write(`nx: no such file: ${files[0]} — try \`nx --help\`\n`); process.exit(1); }
    if (!st.isFile()) { process.stderr.write(`nx: not a file: ${files[0]}\n`); process.exit(1); }
    runFile(files[0], args.includes('--trace'), args.includes('--strict'));
  } catch (e) {
    process.stderr.write((e instanceof NxError ? e.format() : `nx: ${e.message}`) + '\n');   // never leak a raw Node stack
    process.exit(1);
  }
}

...
// The ROADMAP thesis: Myxo makes a model write correct, *safe* agent-code more reliably than Python-in-a-sandbox.
...
//                    (same anti-hardcode rule as the Myxo checks).
// A task WITHOUT a control/ dir is skipped and reported as 'no-control' (honest: not every bucket maps to a
// fair plain-Python control — fence/concurrency are the Myxo-differentiated buckets; see BASELINE.md).
//
// Scoring runs solution.py + check.py together in a FRESH python subprocess with a wall-clock timeout
// (the analog of the Myxo harness's fuel bound — a non-terminating solution can't hang the run).
const fs = require('fs');
...
// control is a coding-correctness measure, not a security surface (that's the fence bucket's job, in Myxo).
...
async function main() {
  ...
  for (const t of tasks) {
    let r;
    if (!t.hasControl) { r = { pass: false, errClass: 'no-control', detail: 'no python control for this task (Myxo-differentiated bucket)' }; }
    else {

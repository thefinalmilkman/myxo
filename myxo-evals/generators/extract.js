...
//   1. prefers ```myx / ```nexus blocks (concatenates ALL of them, in order),
...
function extractNx(text) {
  ...
  const nxBlocks = fences.filter(m => /^(myx|myxo|nx|nexus)$/i.test(m[1]));
  const chosen = nxBlocks.length ? nxBlocks : fences;      // prefer myx-tagged (nx/nexus kept for back-compat); else take every block

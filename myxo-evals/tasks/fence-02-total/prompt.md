Write an Myxo script (not just an agent) that the host runs with `requireManifest` on. The host grants two natives: `charge` (returns a string) and `spend` (returns a string), and meters `spend` **by value** (its numeric first argument is the amount spent).

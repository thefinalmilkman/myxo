Write an Myxo script (not just an agent) that the host runs with `requireManifest` on. The host grants two natives: `lookup` (returns a string) and `notify` (returns a string). Neither is value-metered, so a `total` in the manifest caps the **number of calls**.

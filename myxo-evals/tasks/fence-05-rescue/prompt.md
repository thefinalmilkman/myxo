Write an Myxo script that the host runs with `requireManifest` on. The host grants two natives: `pay` (returns a string) and `log` (returns a string), and meters `pay` **by value** (its numeric first argument is the amount).

'use strict';
// nx-live.js — run an Nx script against ASYNC tools, synchronously.
//
// Nx's interpreter is synchronous (a tree-walker), but real-world tools — MCP calls,
...
// the parent posts the result back, so from inside Nx the call looks like a plain value.
//
// The transport is abstract: you supply `onCall(name, argsObject) -> Promise<result>`.
// Whether that's an in-process MCP dispatch, an HTTP fetch, or a db query, nx-live does
...
function runLive(script, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(path.join(__dirname, 'nx-live-worker.js'), {
    ...
    timer = setTimeout(() => {
      finish(resolve, { ok: false, output: '', audit: [], error: `Nx run timed out after ${timeoutMs}ms` });
    }, timeoutMs);
    ...
    worker.on('exit', (code) => { if (!settled && code !== 0) finish(reject, new Error('nx worker exited ' + code)); });
  });
}

// james-nx-demo.js — exactly what the `james_nx_run` MCP handler does, runnable WITHOUT
...
//   node examples/james-nx-demo.js
'use strict';
const { runLive } = require('../nx-live');

const tick = () => new Promise((r) => setTimeout(r, 3));

// stand-ins for the read/approval-only JAMES Nx surface.
const HANDLERS = {
...
const onCall = async (name, a) => {
  ...
  if (!h) throw new Error(`tool '${name}' is not available to Nx scripts`);

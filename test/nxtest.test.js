'use strict';
// nxtest.test.js — node:test coverage for the `nx test` runner (the meta-layer).
// The runner itself is then dogfooded by tests/lang.test.nx (Nx testing itself in Nx).
const { test } = require('node:test');
const assert = require('node:assert/strict');
const fs = require('fs');
const path = require('path');
const { run, runTests } = require('../nx');
...
test('test blocks are inert in a normal run (not via nx test)', () => {
...
test('the self-hosted Nx suite (tests/lang.test.nx) passes', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'tests', 'lang.test.nx'), 'utf8');

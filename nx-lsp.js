'use strict';
// nx-lsp.js — a minimal Language Server for Nx (stdio JSON-RPC, LSP 3.x).
...
const KEYWORD_DOCS = {
  ...
  test: 'test "name" { expect ... } — a test case (runs under `nx test`).',
...
function hoverAt(text, line, character) {
  ...
  if (builtinNames().includes(w)) return `${w} — Nx builtin/stdlib agent.`;
  return null;
}
  ...
  switch (method) {
    case 'initialize':
      return { response: { id, result: {
        serverInfo: { name: 'nx-lsp', version: '1.0' },
        capabilities: {

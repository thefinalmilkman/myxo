# nx-evals — the Nx eval harness (Phase 1)

The instrument. Everything in the Road to 1.0 (`../ROADMAP.md`) claims to make Nx more AI-writable; this is the only thing that can *prove* it.
...
- `solution.nx` — the **reference** solution (proves the task is solvable; used by `--ref`).
- **one checker:**
  - `check.nx` — Nx `test`/`expect` blocks referencing the solution's agents (run via `nx.runTests`), **or**
...
The product metric is the **Python delta**: the same 30 tasks written by the same model in Python-in-a-sandbox. If a model writes safe Python as reliably as Nx, the thesis is weaker than believed (see ROADMAP kill criteria). Wire a `python` scorer + `generators/*` python path to produce it.

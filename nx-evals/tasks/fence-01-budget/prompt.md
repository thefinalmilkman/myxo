Write an Nx script (not just an agent) that the host runs with `requireManifest` on. The host grants two natives: `lookup` (returns a string) and `spend` (returns a string). The script must:

'use strict';
// Frontier tier via the Anthropic Messages API — zero deps, plain https (matches nx-lang's no-dep law).
...
const MAX_TOKENS = 4096; // Nx solutions are short; non-streaming is safe at this size
...
exports.generate = function (task, opts) {
  return new Promise((resolve, reject) => {
    ...
    const body = JSON.stringify({
      ...
      system: 'You write ONLY Nx code — no prose, no explanation. Nx is a language you have never seen; it is defined ENTIRELY by the spec the user provides. Do not assume Python/JS semantics. Return ONE ```nx code block containing only the Nx solution (define exactly the agents the task names).',
      messages: [{
        role: 'user',
        content: 'Nx language spec:\n\n' + spec + '\n\n=== TASK ===\n' + task.prompt +
          '\n\nReturn ONE ```nx code block with only the solution.',
      }],
    });

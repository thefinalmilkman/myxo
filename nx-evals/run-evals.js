#!/usr/bin/env node
'use strict';
// Nx eval harness (Phase 1). The instrument: does a model (or a reference solution) produce Nx
...
//
// A task folder holds prompt.md + (check.nx  OR  expected.txt [+ host.js]) + solution.nx (the reference)
//   + optional `requires` (newline-separated tokens that MUST appear in the solution source — the construct gate).
// Type A (check.nx): solution + check are run via nx.runTests; pass = all check expects pass.
// Type B (expected.txt): solution is run via nx.run (host.js grants natives); pass = captured output matches
...
const nx = require(path.join(__dirname, '..', 'nx'));
...
function score(task, solutionSrc) {
  ...
  if (task.check != null) {                                   // Type A — checks.nx via runTests
    const graded = stripTests(solutionSrc) + '\n\n' + task.check;   // drop the model's own tests; score only the check's
    let res;
    try { res = nx.runTests(graded, { dir: task.dir, output: () => {}, maxSteps: MAX_STEPS }); }
  ...
  if (task.expected != null) {                                // Type B — expected.txt via run() + host natives
    ...
    let out, ledger = [];
    try { out = nx.run(solutionSrc, { capture: true, natives, valueCaps, dir: task.dir, requireManifest: !!natives, maxSteps: MAX_STEPS, onAudit: l => { ledger = l || []; } }); }
    catch (e) { return { pass: false, errClass: 'runtime', detail: String((e && e.message) || e).slice(0, 180) }; }
  ...
  return { pass: false, errClass: 'no-check', detail: 'task has neither check.nx nor expected.txt' };
}

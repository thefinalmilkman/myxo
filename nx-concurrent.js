'use strict';
// nx-concurrent.js — the parent half of Nx parallelism. Spawns one worker per task, runs them on real OS
// threads, and blocks the calling thread on an Atomics barrier until all finish — so the synchronous `gather`
// gets genuine multi-core parallelism with no callbacks. Built on the same worker+Atomics pattern as nx-live.
//
// runParallel(tasks, opts) -> [results] (plain JS values), in input order.
//   tasks[i] = { name, params, body, args }   args are plain JS (already nx->js converted)
...
// not the mechanism: a worker posts its message BEFORE ticking the barrier (see nx-par-worker.js), so once
...
function runSettled(tasks, opts = {}) {
  ...
  try {
    for (let i = 0; i < n; i++) {
      ...
      const t = tasks[i];
      const w = new Worker(path.join(__dirname, 'nx-par-worker.js'), {
      ...
      // any JS at all ticks the barrier itself from a `finally` (nx-par-worker.js); a true hard death (OOM,
